...
if (1) {
  ...
  # add a meaningless continuous variable 
  x = cbind(rnorm(n), x)
  xtest = cbind(rnorm(nt), xtest)
...
# max_depth = matrix(3, ntrees, nsweeps)
...
fit = XBART(as.matrix(y), as.matrix(x), as.matrix(xtest), M, L, nsweeps, max_depth, Nmin, alpha = alpha, beta = beta, p_categorical = dim(x)[2] - 1, tau = tau, s = 1, kap = 1, mtry = mtry, burnin = burnin, m_update_sigma = TRUE, draw_mu = TRUE, Ncutpoints = Ncutpoints, parallel = FALSE)
...
cat("RMSE of BART ", RMSE1, " running time ", t1, "\n")
cat("RMSE of XBART ", RMSE2, " running time ", t2, "\n")