...
fit = XBART.multinomial(y = matrix(y_train), num_class = 3, X = X_train, Xtest = X_test, 
    ...
    set_random_seed = FALSE, random_seed = NULL, sample_weights_flag = TRUE)
...
tm = proc.time()
fit.xgb <- xgboost(data = X_train, label = y_train, num_class = 3, verbose = 0, max_depth = 4, 
    subsample = 0.8, nrounds = 500, early_stopping_rounds = 2, eta = 0.1, params = list(objective = "multi:softprob"))
...
cat(paste("ranger rmse on probabilities: ", round(sqrt(mean((pred3 - pr)^2)), 3)), 
    "\n")
cat(paste("xgboost rmse on probabilities: ", round(sqrt(mean((phat.xgb - pr)^2)), 
    3)), "\n")

# par(mfrow=c(1,3)) plot(pred3[,1],pr[,1],pch=20,cex=0.5)
# plot(pred3[,2],pr[,2],pch=20,cex=0.5) plot(pred3[,3],pr[,3],pch=20,cex=0.5)
