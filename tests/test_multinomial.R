...
fit <- XBART.multinomial(
    y = y_train, num_class = 3, X = X_train, Xtest = X_test,
...
# predict by a separate function, only look at the 4 - 10th forest
pred <- predict(fit, X_test, 4:10)

# check that both predictions are the same
fit$yhats_test[4:10, , ] == pred$yhats
