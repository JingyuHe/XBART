...
fit = XBART(as.matrix(y), as.matrix(x), as.matrix(xtest), M, L, nsweeps, max_depth, Nmin, alpha = alpha, beta = beta, p_categorical = dim(x)[2], tau = tau, s = 1, kap = 1, mtry = mtry, burnin = burnin, m_update_sigma = TRUE, draw_mu = TRUE, Ncutpoints = Ncutpoints, parallel = FALSE)
