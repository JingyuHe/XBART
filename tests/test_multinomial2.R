...
# thresh <- sort(pihat) N <- length(pihat) yhat <- sapply(1:N, function(a)
# as.double(pihat >= thresh[a])) tpr <- sapply(1:N, function(a)
# length(which(ytrue==1 & yhat[,a]==1))/sum(ytrue==1)) fpr <- sapply(1:N,
# function(a) length(which(ytrue==0 & yhat[,a]==1))/sum(ytrue==0)); if (add ==
# FALSE){ plot(fpr,tpr,pch=20,cex=0.8,col=col,bty='n',type='b')
# abline(a=0,b=1,lty=2) }else{
# points(fpr,tpr,pch=20,cex=0.8,col=col,bty='n',type='b') } #print(mean(tpr)) }
...
fit = XBART.multinomial(y = y_train, num_class = 3, X = X_train, Xtest = X_test, 
    num_trees = num_trees, num_sweeps = num_sweeps, max_depth = 250, Nmin = 1, num_cutpoints = 100, 
    alpha = 0.95, beta = 2, tau = 8/num_trees, no_split_penality = "Auto", burnin = 1L, 
    mtry = p, p_categorical = 0L, kap = 1, s = 1, verbose = TRUE, parallel = FALSE, 
    set_random_seed = FALSE, random_seed = seed, sample_weights_flag = TRUE)

# number of sweeps * number of observations * number of classes
# dim(fit$yhats_test) tm = proc.time()-tm print(tm)
