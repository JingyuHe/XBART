...
XBART.multinomial <- function(y, num_class, X, num_trees = 20, num_sweeps = 20, max_depth = 20, Nmin = NULL, num_cutpoints = NULL, alpha = 0.95, beta = 1.25, tau_a = 1, tau_b = 1, 
    no_split_penalty = NULL, burnin = 5, mtry = NULL, p_categorical = 0L, verbose = FALSE, parallel = TRUE, random_seed = NULL, sample_weights = TRUE, separate_tree = FALSE, 
    weight = 1, update_weight = TRUE, update_tau = TRUE, update_phi = TRUE, nthread = 0, a = 0.0001, weight_exponent = 3, MH_step = 0.5, tree_size = 100, extra_trees = 100,...) {
    require(GIGrvg)
    ...
    obj <- XBART_multinomial_cpp(y, num_class, X, num_trees, num_sweeps, max_depth, Nmin, num_cutpoints, alpha, beta, tau_a, tau_b, no_split_penalty, burnin, mtry, 
        p_categorical, verbose, parallel, set_random_seed, random_seed, sample_weights, separate_tree, weight, update_weight, update_tau, update_phi, nthread,
        a, weight_exponent, MH_step, tree_size, extra_trees)
