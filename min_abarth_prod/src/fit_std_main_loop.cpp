...
void fit_std_main_loop(const double *Xpointer,std::vector<double> &y_std,double y_mean,const double *Xtestpointer, xinfo_sizet &Xorder_std,
    ...
    xinfo &yhats_xinfo,xinfo &yhats_test_xinfo,xinfo &sigma_draw_xinfo,vector<tree> &trees)
{
    ...
    for (size_t mc = 0; mc < L; mc++)
    {
        ...
        for (size_t sweeps = 0; sweeps < N_sweeps; sweeps++)
        {
            ...
            for (size_t tree_ind = 0; tree_ind < M; tree_ind++)
            {
                ...
                trees[tree_ind].grow_tree_adaptive_abarth_train(sum_vec(residual_std) / (double)N, 0, max_depth_std[sweeps][tree_ind], Nmin, Ncutpoints, tau, sigma, alpha, beta, draw_sigma, draw_mu, parallel, residual_std, Xorder_std, Xpointer, mtry, run_time, use_all, mtry_weight_current_tree, split_count_current_tree);
                ...
                // fit_new_std(trees.t[tree_ind], Xtestpointer, N_test, p, predictions_test_std[tree_ind]);
                 ...
                 // update prediction of current tree
                fit_new_std(trees[tree_ind], Xpointer, N, p, predictions_std[tree_ind]);

                // update prediction of current tree, test set
                fit_new_std(trees[tree_ind], Xtestpointer, N_test, p, predictions_test_std[tree_ind]);
