...
model = XBART(num_trees = 10,num_sweeps = 150,n_min = 10,burnin = 25)
...
xbart_hack_score = accuracy_score(valid_fe['Survived'],phat_test>0)
...
preds = model.predict(valid_fe.drop("Survived",axis=1))
phat_test = preds.mean(axis=1)
xbart_clt_score = accuracy_score(valid_fe['Survived'],phat_test>0)

print("Fit XBART Probit")
model = XBART(num_trees = 10,num_sweeps = 150,n_min = 10,burnin = 25,model = "Probit")
model.fit(train_fe.drop("Survived",axis=1),train_fe["Survived"]*2-1)
preds = model.predict(valid_fe.drop("Survived",axis=1))
phat_test = preds.mean(axis=1)
xbart_probit_score = accuracy_score(valid_fe['Survived'],norm.cdf(phat_test)>0.5)
