...
void hskXBCFDiscreteModel::updateNodeSuffStat(State &state,
...
void hskXBCFDiscreteModel::state_sweep(size_t tree_ind, size_t M, State &state, X_struct &x_struct) const
{
...
void hskXBCFDiscreteModel::predict_std(const double *Xtestpointer, size_t N_test, size_t p, size_t num_trees, size_t num_sweeps, matrix<double> &yhats_test_xinfo, vector<vector<tree>> &trees)
{
...
void hskXBCFDiscreteModel::store_residual(State &state)
{
    for (size_t i = 0; i < (*state.residual_std)[0].size(); i++)
    {
