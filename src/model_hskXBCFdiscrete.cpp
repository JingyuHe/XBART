    ...
    for (size_t sweeps = 0; sweeps < num_sweeps; sweeps++)
    {
        for (size_t data_ind = 0; data_ind < N_test; data_ind++)
        {
            ...
            for (size_t i = 0; i < trees_mod[0].size(); i++)
            {
                ...
                treatment_leaf(sweeps, data_ind, i) = leaf_mod[i][0];
            }

            for (size_t i = 0; i < trees_con[0].size(); i++)
            {
                prognostic_xinfo[sweeps][data_ind] += output_con[i][0];
                prognostic_leaf(sweeps, data_ind, i) = leaf_con[i][0];
            }

            // scaling by a and b will be calculated in the R script, see predict.XBART.R file
            // the lines below are useless
