...
void mcmc_loop_multinomial(matrix<size_t> &Xorder_std, bool verbose, vector<vector<tree>> &trees, double no_split_penalty,
                           ...
                           std::vector< std::vector<double> > &tau_samples, std::vector< std::vector<double> > &weight_samples, std::vector< std::vector<double> > &entropy, double entropy_threshold, size_t &num_stops)
{
    // if (state->parallel)
    //     thread_pool.start();
