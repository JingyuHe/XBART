    ...
    for (size_t sweeps = 0; sweeps < num_sweeps; sweeps++)
    {
        ...
        for (size_t tree_ind = 0; tree_ind < num_trees; tree_ind++)
        {
            ...
            if(sweeps > 1){
                ...
                temp_tree[tree_ind].update_split_prob(fit_info, sum_vec(fit_info->residual_std) / (double)N, 0, max_depth_std[sweeps][tree_ind], tau, sigma, alpha, beta, draw_mu, parallel, Xorder_std, mtry, mtry_weight_current_tree, fit_info->X_counts, fit_info->X_num_unique, model, tree_ind, sample_weights_flag);
