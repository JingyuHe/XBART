...
void tree::grow_tree_adaptive_linkedlist(double y_mean, double y_sum, size_t depth, size_t max_depth, size_t Nmin, size_t Ncutpoints, double tau, double sigma, double alpha, double beta, bool draw_sigma, bool draw_mu, bool parallel, std::vector<double> &y_std, xinfo_sizet &Xorder_std, const double *X_std, double *split_var_count_pointer, size_t &mtry, const std::vector<size_t> &subset_vars, double &run_time, xinfo_sizet &Xorder_next_index, xinfo_sizet &Xorder_full, std::vector<size_t> &Xorder_firstline, double& old_time, double& new_time, size_t& N_Xorder)
{
    ...
    // cout << "ok1 " << endl;
    BART_likelihood_adaptive_linkedlist(y_mean * N_Xorder, y_std, X_std, tau, sigma, depth, Nmin, Ncutpoints, alpha, beta, no_split, v, split_point, parallel, subset_vars, Xorder_full, Xorder_next_index, Xorder_firstline, N_y, c, old_time, new_time, N_Xorder);
    // cout << "ok2 " << endl;
    if (no_split == true)
    {
        return;
    ...
    depth = depth + 1;
    tree::tree_p lchild = new tree();
    lchild->grow_tree_adaptive_linkedlist(yleft_mean_std, yleft_sum, depth, max_depth, Nmin, Ncutpoints, tau, sigma, alpha, beta, draw_sigma, draw_mu, parallel, y_std, Xorder_left_std, X_std, split_var_count_pointer, mtry, subset_vars, running_time_left, Xorder_next_index, Xorder_full, Xorder_left_firstline, old_time, new_time, N_Xorder_left);
    tree::tree_p rchild = new tree();
    rchild->grow_tree_adaptive_linkedlist(yright_mean_std, yright_sum, depth, max_depth, Nmin, Ncutpoints, tau, sigma, alpha, beta, draw_sigma, draw_mu, parallel, y_std, Xorder_right_std, X_std, split_var_count_pointer, mtry, subset_vars, running_time_right, Xorder_next_index, Xorder_full, Xorder_right_firstline, old_time, new_time, N_Xorder_right);
