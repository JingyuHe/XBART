...
void logNormalModel::samplePars(std::unique_ptr<State> &state,
                                ...
                                double &prob_leaf)
{
    std::gamma_distribution<double> gammadist(alpha + 0.5 * suff_stat[1], 1.0 / (beta + 0.5 * suff_stat[0]));
    theta_vector[0] = gammadist(state->gen);
    return;
}

// UNNEDED: we don't draw sigma here
                                        ...
                                        size_t row_ind)
{
    ...
    //suff_stat[0] += pow(residual_std[0][Xorder_std[split_var][row_ind]], 2);
    //suff_stat[1] += 1;
    return;
}
/*
                                  ...
                                  std::unique_ptr<State> &state) const
{
    ...
    return -(alpha + n_b) * log(beta + res_sum_b) + lgamma(alpha + n_b);
}

void logNormalModel::ini_residual_std(std::unique_ptr<State> &state, matrix<double> &mean_residual_std)
{
    // initialize partial residual at the residual^2 from the mean model
    for (size_t i = 0; i < state->residual_std[0].size(); i++)
    {
        state->residual_std[0][i] = pow(mean_residual_std[0][i], 2);
    }
    return;
}
...
void logNormalModel::update_sigmas(matrix<double> &mean_residual_std, matrix<double> &var_residual_std)
{
    // initialize partial residual at the residual^2 from the mean model
    for (size_t i = 0; i < mean_residual_std[0].size(); i++)
    {
        mean_residual_std[1][i] = 1.0 / var_residual_std[0][i];
