    ...
    for (size_t sweeps = 0; sweeps < mean_state.num_sweeps; sweeps++)
    {
       ...
       // NK: loop for the mean model forest
        for (size_t tree_ind = 0; tree_ind < mean_state.num_trees; tree_ind++)
        {
            ...
            // single core
            mean_trees[sweeps][tree_ind].grow_from_root(mean_state, Xorder_std, mean_x_struct.X_counts, mean_x_struct.X_num_unique, mean_model, mean_x_struct, sweeps, tree_ind);
                    ...
                    matrix<double> &res_v)
{
    ...
    for (size_t sweeps = 0; sweeps < mean_state.num_sweeps; sweeps++)
    {
        ...
        for (size_t tree_ind = 0; tree_ind < mean_state.num_trees; tree_ind++)
        {
            ...
            if (sweeps >= mean_state.burnin)
            {
                for (size_t i = 0; i < (*mean_state.split_count_all).size(); i++)
                {
                    (*mean_state.split_count_all)[i] += (*mean_state.split_count_current_tree)[i];
                }
            }
                    ...
                    )
{
    ...
    for (size_t sweeps = 0; sweeps < state.num_sweeps; sweeps++)
    {
        ...
        var_model->update_state2(state, state.num_trees, var_x_struct);
