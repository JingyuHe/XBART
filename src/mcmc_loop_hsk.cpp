                    ...
                    std::unique_ptr<X_struct> &var_x_struct)
{
    ...
    COUT << mean_state->ini_var_yhat << "< varyhat | numtrees >" << endl;//<< (double)mean_state->num_trees << endl;
    ...
    for (size_t sweeps = 0; sweeps < mean_state->num_sweeps; sweeps++)
    {
        ...
        for (size_t tree_ind = 0; tree_ind < mean_state->num_trees; tree_ind++)
        {
            ...
            if (mean_state->parallel)
            {
            // NK: these all comments below should probably be removed
                // run parallel
...
// #pragma omp section
        ...
        var_model->ini_residual_std(var_state, mean_state->residual_std);
        ...
        for (size_t tree_ind = 0; tree_ind < var_state->num_trees; tree_ind++)
        {
            ...
            // subtract old tree for sampling case
            // NK: this may not work the same for logNormal model (comment it for now)
            //if (var_state->sample_weights_flag)
            //{
            //    var_state->mtry_weight_current_tree = var_tate->mtry_weight_current_tree - var_state->split_count_all_tree[tree_ind];
            //}

            var_model->initialize_root_suffstat(var_state, var_trees[sweeps][tree_ind].suff_stat);

            if (var_state->parallel)
            {
            // NK: these all comments below should probably be removed
                // run parallel
                // omp_set_max_active_levels(state->nthread);
...
// #pragma omp section
        ...
        var_model->update_sigmas(mean_state->residual_std, var_state->residual_std);

        // NK: do we store sigma values here?
