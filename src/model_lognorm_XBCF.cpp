...
void logNormalXBCFModel::ini_residual_std(State &state, matrix<double> &mean_residual_std, X_struct &x_struct_v_con, X_struct &x_struct_v_mod)
{
    ...
    for (size_t i = 0; i < (*state.residual_std)[0].size(); i++)
    {
        ...
        {
            // control group
            (*state.residual_std)[0][i] = 2 * log(abs(mean_residual_std[0][i])) + log((*state.precision)[i]) - log((*(x_struct_v_con.data_pointers[0][i]))[0]);
        }
    }
    return;
}
...
void logNormalXBCFModel::ini_residual_std2(State &state, X_struct &x_struct_v_con, X_struct &x_struct_v_mod)
{
    ...
    for (size_t i = 0; i < (*state.residual_std)[0].size(); i++)
    {
        ...
        {
            (*state.residual_std)[0][i] = 2 * log(abs((*state.residual_std)[0][i])) + log((*state.precision)[i]) - log((*(x_struct_v_con.data_pointers[0][i]))[0]);
        }
    }
    return;
}

void logNormalXBCFModel::initialize_root_suffstat(State &state, std::vector<double> &suff_stat)
{
    suff_stat[0] = 0;
    suff_stat[1] = 0;
...
void logNormalXBCFModel::incSuffStat(State &state,
                                     ...
                                     std::vector<double> &suffstats)
{
    ...
    {
        // trees for control group
        if (!((*state.Z_std)[0][index_next_obs]))
        {
            // if the observation is control
            suffstats[0] += exp((*state.residual_std)[0][index_next_obs]);
            suffstats[1] += 1;
        }
    }
...
void logNormalXBCFModel::updateNodeSuffStat(State &state,
...
void logNormalXBCFModel::samplePars(State &state,
                                    std::vector<double> &suff_stat,
...
void logNormalXBCFModel::calculateOtherSideSuffStat(std::vector<double> &parent_suff_stat, std::vector<double> &lchild_suff_stat, std::vector<double> &rchild_suff_stat, size_t &N_parent, size_t &N_left, size_t &N_right, bool &compute_left_side)
{
...
double logNormalXBCFModel::likelihood(std::vector<double> &temp_suff_stat,
...
void logNormalXBCFModel::state_sweep(State &state,
                                     size_t tree_ind,
                                     size_t M,
                                      ...
                                      X_struct &x_struct_v_mod)
{
    ...
    for (size_t i = 0; i < (*state.residual_std)[0].size(); i++)
    {
        ...
        else
        {
            for (size_t j = 0; j < tree_ind; j++)
            {
                log_sigma2 += log((*(x_struct_v_con.data_pointers[j][i]))[0]);
            }
        }
...
void logNormalXBCFModel::switch_state_params(State &state)
{
